...
public:
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        ...
        for (string s : strs) {
            // Build array with counts of letters
            int a[26] = {0};
            for (char i: s) a[i - 'a']++;

            // Create hash of combination (string of 26 chars)
            string hash = "";
            for (int i : a) {
            ...
            // Check if hash is in index, else add it and add it to solution
            if (!index.count(hash)) {
                index[hash] = sol.size();
                vector<string> new_anagram;
                new_anagram.push_back(s);
                sol.push_back(new_anagram);
            } else {
                sol[index.at(hash)].push_back(s);
            }
        }
        return sol;
    }
...
int main() {
    ...
    vector<string> x = {"bdddddddddd","bbbbbbbbbbc"};
    print_vv(sol.groupAnagrams(x));
