...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        sort(nums.begin(), nums.end()); // Sort the numbers in nlog(n)
        vector<vector<int>> solution;
        for (int i=0;i<nums.size();i++) {
            if (i > 0 && nums[i] == nums[i-1]) continue; // Avoid duplicate answers
            int l = i+1;
            int r = nums.size()-1;
            while (l<r) {
                ...
                if (sum > 0) r--;
                    ...
                    solution.push_back({nums[l],nums[i],nums[r]});
                    l++;
