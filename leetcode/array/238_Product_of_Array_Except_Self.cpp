...
public:
    vector<int> productExceptSelf(vector<int>& nums) {
        // build left prefix
        int lprefix[nums.size()];
        int prefix_value = 1;
        for (int i=0;i<nums.size();i++) {
            prefix_value *= nums[i];
            lprefix[i] = prefix_value;
        }

        // build right prefix
        int rprefix[nums.size()];
        prefix_value = 1;
        for (int i=nums.size()-1;i>=0;i--) {
            prefix_value *= nums[i];
            rprefix[i] = prefix_value;
        }
        ...
        for (int i=0;i<nums.size();i++) {
            if (i == 0) {
                ans.push_back(rprefix[i+1]);
            } else if (i == nums.size()-1) {
                ans.push_back(lprefix[i-1]);
            } else {
                ans.push_back(lprefix[i-1]*rprefix[i+1]);
            }
        }
...
int main() {
    ...
    Solution sol;
    vi x = {1, 2, 4, 6};
    print_v(sol.productExceptSelf(x));
