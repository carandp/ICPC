...
public:
    vector<int> minBitwiseArray(vector<int>& nums) {
        ...
        for (int i=0;i<n;i++) {
            int p = nums[i];
            if (p == 2) { // No bit right to the first 0
                res.push_back(-1);
                continue;
            }
            int firstPos0 = 1;
            while ((p&firstPos0) != 0) { // Increment till first 0 is found
                firstPos0 <<= 1;
            }
            firstPos0 >>= 1; // Go 1 bit to the right of the first 0
            res.push_back(p-firstPos0);
        }
        return res;
    }
};
