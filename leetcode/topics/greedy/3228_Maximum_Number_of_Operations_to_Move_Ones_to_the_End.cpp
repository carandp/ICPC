...
public:
    int maxOperations(string s) {
        s = s + "1"; // Add 1 to end to solve edge cases: 00100
        int operations = 0;
        int numOnes = 0;
        bool gap = false;
        for (int i=0;i<s.size();i++) {
            int num = s[i] - '0';
            if (!numOnes && !num) continue; // No backlog of ones or a 1 found, continue
            if (numOnes && !num) gap = true; // We found a gap
            if (num && gap) { // Found a 1 after a gap
                operations+=numOnes;
                gap = false;
            }
            if (num) numOnes++; // Add to 1's counter
        }
        return operations;
    }
};
