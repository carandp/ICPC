...
public:
    struct cmp {
        bool operator()(const vector<int>& a, const vector<int>& b) const {
            if (a[0] != b[0])
                return a[0] > b[0];  // Descending by start of range
            return a[1] < b[1]; // Ascending by end of range
        }
    };
    ...
    int intersectionSizeTwo(vector<vector<int>>& intervals) {
        ...
        for (int i=1;i<n;i++) { // Greedy approach
            int l = intervals[i][0];
            int r = intervals[i][1];
            if (x >= l && y <= r) continue; // There is nothing to update, range contains x and y
            if (l < x && r < x) { // Range does not contain x or y
                x = l;
                y = l+1;
                res += 2;
            } else { // Range does not contain y
                y = x;
                x = l;
                res++;
            }
...
int main() {
    ...
    Solution sol;
    vector<vector<int>> x = {{2,7},{1,5},{2,4},{4,6},{0,3},{6,8},{0,8}};
    cout << sol.intersectionSizeTwo(x) << nl;
