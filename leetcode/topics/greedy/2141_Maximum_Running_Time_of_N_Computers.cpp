...
public:
    long long maxRunTime(int n, vector<int>& batteries) {
        ...
        vector<long long> comps(n,0);
        long long extra = 0; // Store extra battery time
        for (int i=0;i<batteries.size();i++) {
            if (i >= batteries.size() - n) comps[i - batteries.size() + n] = batteries[i];
            else extra += batteries[i];
        }
        for (int i=0;i<n-1;i++) {
            if (extra/(i+1) < comps[i+1]-comps[i]) return comps[i] + extra/(i+1); // If we can't add enough extra battery so all i+1 comps run for comp[i+1]-comp[i] extra time
            extra -= (i+1) * (comps[i+1]-comps[i]); // Update extra by reducing extra battery added to all i+1 comps to run for comp[i+1]-comp[i] extra time
        }
        return comps.back() + extra/n;
    }
};
...
int main() {
    ...
    Solution sol;
    vi x = {18,54,2,53,87,31,71,4,29,25};
    int n = 9;
    cout << sol.maxRunTime(n,x) << nl;
