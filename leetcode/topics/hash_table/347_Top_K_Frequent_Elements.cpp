...
public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        // Group all numbers in frequency
        for (auto &p : frequency) {
            bucket[p.second].push_back(p.first);
        }

        // Obtain the k elements going from nums.length max to 1
        for (int i = 10000; i >= 1; i--) {
            if (bucket.count(i)) {
                int j = 0;
                while (k && j < bucket[i].size()) {
                    solution.push_back(bucket[i][j]);
                    j++;
                    k--;
                }
            }
