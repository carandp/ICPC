...
public:
    int countPartitions(vector<int>& nums, int k) {
        ...
        dp[0] = 1;
        prefixSumDp[0] = 1;
        int j = 0; // Slicing window from [j,i]

        for (int i=0;i<n;i++) {
            int num = nums[i];
            ms.emplace(num);
            while (j <= i && *ms.rbegin() - *ms.begin() > k) { // Adjust slicing window
                ms.erase(ms.find(nums[j]));
                j++;
            }
            dp[i+1] = prefixSumDp[i];
            if (j) dp[i+1] -= prefixSumDp[j-1]; // dp[i+1] = prefixSumDp[i] - prefixSumDp[j-1]
            dp[i+1] += mod;
            dp[i+1] %= mod;
