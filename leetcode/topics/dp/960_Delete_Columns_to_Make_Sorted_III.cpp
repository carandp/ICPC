...
class Solution {
public:
    bool comp (vector<string>& strs, int n, int c1, int c2) {
        for (int i=0;i<n;i++) {
            if (strs[i][c2] > strs[i][c1]) return false;
        }
        return true;
    }
    ...
    int minDeletionSize(vector<string>& strs) {
        int n = strs.size();
        int l = strs[0].size();
        vector<int> dp(l,1); // dp[i]: i = longest increasing subsequence of sorted columns for i-column
        int maxLIS = 1;
        for (int i=0;i<l;i++) {
            for (int j=0;j<i;j++) {
                if (comp(strs,n,i,j)) dp[i] = max(dp[i],dp[j]+1);
            }
