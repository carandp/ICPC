...
public:
    ...
    long long dfs(TreeNode* node) {
        ...
        long long currSum = node->val + l + r;
        subtreeSum.push_back(currSum);
        return currSum;
    }
    ...
    int maxProduct(TreeNode* root) {
        ...
        if (!root) return 0;
        dfs(root);
        int n = subtreeSum.size();
        long long total = *subtreeSum.rbegin();
        long long res = 0;
        for (int i=0;i<n-1;i++) {
            long long currMul = (total-subtreeSum[i])*subtreeSum[i];
            res = max(res,currMul);
        }
        return res%mod;
    }
