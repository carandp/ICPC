...
public:
    int nextBeautifulNumber(int n) {
        ...
        int index = to_string(n).length();
        vector<int> sub_key = key[index]; // Store the vector of ints that contains res
        if (sub_key.back() <= n) sub_key=key[index+1];

        int res = sub_key.back();

        for (int base: sub_key) {
            vector<int> permutation;
            int permutation_int = 0;
            for (char i: to_string(base)) {
                permutation.push_back(i - '0');
            }

            while (permutation_int <= n && permutation_int != base) { // Explore all permutations and find minimum that is > n
                next_permutation(permutation.begin(),permutation.end());
                string s_curr = "";
                for (int i: permutation) {
                    s_curr += to_string(i);
                }
                permutation_int = stoi(s_curr);
            }
            if (base > n && base < permutation_int) permutation_int = base;
            if (permutation_int > n) res = min(permutation_int,res);
        }
        return res;
    }
};
...
int main() {
    ...
    Solution sol;
    int x = 34505;
    cout << sol.nextBeautifulNumber(x) << nl;
}
