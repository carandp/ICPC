...
public:
    long long maxSubarraySum(vector<int>& nums, int k) {
        int n = nums.size();
        vector<long long> minPrefixSum(k,LONG_LONG_MAX);
        minPrefixSum[0] = 0L;
        ...
        for (int i=0;i<n;i++) {
            prefixSum += nums[i];
            int r = (i+1)%k;
            if (minPrefixSum[r] != LONG_LONG_MAX) res = max(res,prefixSum-minPrefixSum[r]); // Max possible current sum of subarray
            minPrefixSum[r] = min(minPrefixSum[r],prefixSum); // Update minPrefixSum[r]
        }
...
int main() {
    ...
    // vi nums = {-5,1,2,-3,4};
    // int k = 2;
    cout << sol.maxSubarraySum(nums,k) << nl;
