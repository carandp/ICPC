...
public:
    int numberOfSubstrings(string s) {
        int n = s.size();
        vector<int> prev0(n + 1, -1); // Stores previous position where a 0 appears
        for (int i = 0; i < n; i++) {
            if (i == 0 || s[i - 1] == '0') prev0[i + 1] = i;
            else prev0[i + 1] = prev0[i];
        }
        ...
        for (int i = 1; i <= n; i++) {
            int cnt0 = (s[i - 1] == '0');
            int j = i;
            while (j > 0 && cnt0 * cnt0 <= n) { // For all substrings ending at i-1 whose leftmost zero is prev0[j]
                int cnt1 = (i - prev0[j]) - cnt0;
                if (cnt0 * cnt0 <= cnt1) {
                    cnt += min(j - prev0[j], cnt1 - cnt0 * cnt0 + 1);
                }
                j = prev0[j]; // Jump to previous 0
                cnt0++;
            }
        }
        return cnt;
...
int main() {
    ...
    Solution sol;
    string s = "00011";
    cout << sol.numberOfSubstrings(s) << nl;
