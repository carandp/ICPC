...
class Solution {
public:
    bool dfs(string curr, int i, string nxt, unordered_map<string,vector<char>>& valid, unordered_map<string,bool>& memory) {
        if (curr.size() == 1) return true;
        if (i == curr.size() - 1) {
            if (memory.contains(nxt)) return memory[nxt];
            bool res = dfs(nxt,0,"",valid,memory);
            memory[nxt] = res;
            return res;
        }
        string k = curr.substr(i,2);

        int possible = valid[k].size();
        for (int c=0;c<possible;c++) { // DFS for all possible combinations
            nxt.push_back(valid[k][c]);
            if (dfs(curr,i+1,nxt,valid,memory)) return true;
            nxt.pop_back();
        }
    ...
    bool pyramidTransition(string bottom, vector<string>& allowed) {
        unordered_map<string,vector<char>> valid;
        for (int i=0;i<allowed.size();i++) {
            valid[allowed[i].substr(0,2)].push_back(allowed[i][2]);
        }
        unordered_map<string,bool> memory;
        return dfs(bottom,0,"",valid,memory);
    }
};
...
int main() {
    ...
    Solution sol;
    string b = "AAAA";
    vector<string> a = {"AAB","AAC","BCD","BBE","DEF"};
    cout << sol.pyramidTransition(b,a) << nl;
