...
public:
    ListNode* modifiedList(vector<int>& nums, ListNode* head) {
        ...
        ListNode base(0, head); // Create base node
        ListNode* previous = &base; // Pointer to previous node
        ListNode* curr = head; // Pointer to current node

        while (curr != nullptr) {
            if (s.count(curr->val)) {
                previous -> next = curr -> next; // Previous next node is current next (skip current node)
            } else {
                previous = curr; // Don't change anything and store current node pointer in previous
            }
            curr = curr -> next; // Update current node to advanced in linked list
        }
