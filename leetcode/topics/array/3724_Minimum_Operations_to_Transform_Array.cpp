...
public:
    long long minOperations(vector<int>& nums1, vector<int>& nums2) {
        ...
        long long min_last_index_operations = 1000000;
        for (int i=0;i<nums1.size();i++) {
            if (nums1[i] == nums2.back()) min_last_index_operations = 1;
            operations += abs(nums2[i] - nums1[i]);
            if (min_last_index_operations > 1 + abs(nums2.back() - nums1[i])) {
                min_last_index_operations = 1 + abs(nums2.back() - nums1[i]);
            }
            if (min_last_index_operations > 1 + abs(nums2.back() - nums2[i])) {
                min_last_index_operations = 1 + abs(nums2.back() - nums2[i]);
            }
            if (nums2[i] > nums1[i]) {
                if (nums2[i] >= nums2.back() && nums1[i] <= nums2.back()) min_last_index_operations = 1;
            } else {
                if (nums2[i] <= nums2.back() && nums1[i] >= nums2.back()) min_last_index_operations = 1;
            }
        }
        return operations + min_last_index_operations;
    }
};
...
int main() {
    ...
    // vi x = {753,357};
    // vi x2 = {271,520,413};
    cout << sol.minOperations(x,x2) << nl;
