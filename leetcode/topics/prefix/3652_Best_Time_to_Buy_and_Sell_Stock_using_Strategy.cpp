...
public:
    long long maxProfit(vector<int>& prices, vector<int>& strategy, int k) {
        ...
        for (int i=0;i<n;i++) {
            ...
            prefixProfit[i+1] = prefixProfit[i] + (prices[i]*1LL*strategy[i]);
        }
        long long res = prefixProfit[n];
        for (int l=1;l<n-k+2;l++) {
            int m = l + (k/2);
            int r = l + k - 1;
            long long newProfit = prefixProfit[l-1] + prefixProfit[n] - prefixProfit[r];
            newProfit += prefixSum[r] - prefixSum[m-1];
            res = max(res,newProfit);
        }
        return res;
    }
};
