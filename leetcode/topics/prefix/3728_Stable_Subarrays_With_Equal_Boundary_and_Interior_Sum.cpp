...
public:
    long long countStableSubarrays(vector<int>& capacity) {
        ...
        vector<long long> prefix(capacity.size()); // Build prefix sum
        prefix[0] = capacity[0];
        for (int i=1;i<capacity.size();i++) {
            prefix[i]=prefix[i-1]+capacity[i];
        }

        unordered_map<long long, unordered_map<int, int>> memory; // targetPrefix,capacity
        memory[prefix[0]][capacity[0]]++;

        long long res = 0;

        for (int r=2;r<capacity.size();r++){
            long long targetPrefix = prefix[r-1] - capacity[r];
            res += memory[targetPrefix][capacity[r]];
            memory[prefix[r-1]][capacity[r-1]]++;
        }
        return res;
    }
};
...
int main() {
    ...
    Solution sol;
    // vi x = {-4,4,0,0,-8,-4};
    vi x = {-4,4,0,0,-8,-4,100,-200,300,100,5,5,5};
    // vi x = {9,3,3,3,9};
    // vi x = {3,3,3,0};
    cout << sol.countStableSubarrays(x) << nl;
