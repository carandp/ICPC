...
public:
    int maxBalancedSubarray(vector<int>& nums) {
        ...
        // Use a <difference,prefix> array
        map<pair<int,int>,int> ocurr; // pair<diff,xor> = index
        ocurr[{0,0}] = -1;

        int currDiff = 0; // + when even, - when odd
        int currXOR = 0;
        int res = 0;

        for (int i=0;i<n;i++) {
            if (nums[i]%2) currDiff--;
            else currDiff++;
            currXOR ^= nums[i];
            pair<int,int> curr = {currDiff,currXOR};
            if (ocurr.count(curr)) res = max(res,i-ocurr[curr]);
            else ocurr[curr] = i;
        }
...
int main() {
    ...
    Solution sol;
    // vi x = {3,2,8,5,4,14,9,15};
    // vi x = {3,4,2,1,3,1};
    // vi x = {3,1,3,2,0};
    vi x = {1,2,3,4,5,6,7,8,9,15,16,32,32};
    // vi x = {4,1,2,3,2,2,0,4,2,3,4};
    cout << sol.maxBalancedSubarray(x) << nl;
