...
void solve() {
    ...
    vector<pair<int,int>> kingsInEdge;
    multiset<int> xpos;
    multiset<int> ypos;

    f(i,0,n) {
        pair<int,int> king = kings[i];
        if ((king.first == maxCol) || (king.first == minCol) || (king.second == maxCol) || (king.second == minCol)) {
            kingsInEdge.push_back(king);
            xpos.insert(king.first);
            ypos.insert(king.second);
        }
    }

    ll maxArea = (ll)(maxRow-minRow+1)*(ll)(maxCol-minCol+1);
    f(i,0,kingsInEdge.size()) {
        pair<int,int> king = kingsInEdge[i];
