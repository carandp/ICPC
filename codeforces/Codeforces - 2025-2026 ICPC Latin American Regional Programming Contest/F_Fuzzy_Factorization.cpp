...
void solve() {
    ...
    if (n-10 > 0 ){ // Ignore digits from 10th forward and factorize them into (2*5)^n
        factorization[2] = n-10;
        factorization[5] = n-10;
    }

    ll Y;
    if (n > 10) Y = stoll(S.substr(0,10)); // Get the first 10 digits of the number
    else Y=stoll(S);

    for (ll p=2;p*p<=Y;p++) { // Factorize Y in primes
        while (Y%p == 0) {
            Y/=p;
            factorization[p]++;
        }
    }
    if (Y > 1) factorization[Y]++;

    cout << factorization.size() << nl;
    for (pair<ll,int> i: factorization) {
