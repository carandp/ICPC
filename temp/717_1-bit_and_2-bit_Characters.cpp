...
public:
    bool isOneBitCharacter(vector<int>& bits) {
        int n = bits.size();
        if (n == 1) return true; // If it is only one char
        if (!bits[n-2]) return true; // If it ends in 00
        int i = 0;
        while (i < n-2) { // Greedy approach from left to right
            if (!bits[i]) {
                i++;
            } else i+=2;
        }
        if (i == n-1) return true; // If there is a bit missing, it must be 0 then
        return false;
    }
};
...
int main() {
    ...
    Solution sol;
    vi x = {0,1,1,0};
    cout << sol.isOneBitCharacter(x) << nl;
